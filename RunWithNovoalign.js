...
var novoalign = settings.novoalign;
var novoindex = settings.novoindex;
...
if (process.argv.length > 1 && process.argv[1].substr(process.argv[1].length - 20, process.argv[1].length) === "/RunWithNovoalign.js") {
	if (process.argv.length <= 2) {
		sys.puts("Example usage: 'node RunWithNovoalign.js -g brca1,brca2 -c changesets/changeset.cs -fq /path/to/reads.fq'");
	}
	else {
		var genes = null;
		var changeset = null;
...
function run(genes, changeSet) {
	...
	modifyGenes(genes, changeSet, function(error, createdFiles) {
		...
		combineGenes(createdFiles, changeSet, function(error, fileName) {
			...
			var index = path.dirname(fileName) + "/" + path.basename(fileName, ".fa") + ".ndx";
			//Index with novindex
			console.log("Begin indexing of modified genome " + fileName + "...");
			exec(novoindex + " " + index + " " + fileName, function (error, stdout, stderr) {
				if (error) { return callback(error); }
...
//				exec(bwa + " aln -t " + cores + " " + fileName + " " + fastq + " > " + saiFile, function (error, stdout, stderr) {
				exec(novoalign + " -d " + index + " -f " + fastq + " -o SAM -F ILMFQ  > " + samFile, function (error, stdout, stderr) {
					if (error) { return callback(error); }
...
*/
								...
								//		console.log("Finished indexing reference genome.");
										...
										samFile = path.dirname(saiFile) + "/" + path.basename(saiFile, ".sai") + ".sam";
										...
										path.exists(path.dirname(saiFile), function(exists) {
											...
											var index = path.dirname(referenceGenome) + "/" + path.basename(referenceGenome, ".fa") + ".ndx";
