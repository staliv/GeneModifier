...
if (process.argv.length > 1 && process.argv[1].substr(process.argv[1].length - 17, process.argv[1].length) == "/ShowDeletions.js") {
	if (process.argv.length <= 1) {
		sys.puts("Example usage: 'node ShowDeletions'");
	}
	else {
		showDeletions(function(error, result) {
			if (error) {
				console.dir(error);
			}
...
function showDeletions(callback) {
		...
		if (element.indexOf(".cs") > -1) {
			...
			path.exists(resultsPath + changeset, function(exists) {
				if (exists) {
					...
					getDeletions(resultsPath + changeset + "/", changeSetsPath + changeset + ".cs", function(error, results) {
...
function getDeletions(resultsPath, changesetPath, callback) {
	...
	if (vcfFiles.length === 1) {
		indelsFinder.findIndels(resultsPath + "/" + vcfFiles[0], changesetPath, function(error, result) {
			if (error) { return callback(error); }
			results.modified = correctIndels(result.matches.deletions);
//			results.modified = result.matches.deletions;
